	...
	<div class="row align-self-center mb-2">
		...
		<div id="store-list">
			<c:if test="${!empty slist}">
				<a href="${root}/home/storelist">주변 상권 데이터가 ${ssize}
					개 검색되었습니다.</a>
			</c:if>
		</div>
		<div id="hospital-list">
			<c:if test="${!empty hlist}">
				<a href="${root}/home/hospitallist">주변 안심병원 데이터가 ${hsize}
					개 검색되었습니다.</a>
			</c:if>
		</div>
		<div id="corona-list">
			<c:if test="${!empty clist}">
				<a href="${root}/home/coronalist">주변 코로나 선별 진료소 데이터가 ${csize}
					개 검색되었습니다.</a>
			</c:if>
		</div>
		<div id="pollution-list">
			<c:if test="${!empty plist}">
				<a href="${root}/home/pollutionlist">주변 환경 점검 데이터가 ${psize}
					개 검색되었습니다.</a>
			</c:if>
		</div>
	</div>
